...
public sealed class PaymentsController : BaseApiController
{
        ...
        CancellationToken ct = default)
    {
        ...
        var payments = await _uow.Payments.GetPagedAsync(page, pageSize, search, ct);
        var total = await _uow.Payments.CountAsync(search, ct);
        var items = _mapper.Map<IReadOnlyList<PaymentDto>>(payments);
    ...
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<IActionResult> Update(int id, [FromBody] UpdatePaymentRequest request, CancellationToken ct)
    {
        var existing = await _uow.Payments.GetByIdAsync(id, ct);
        if (existing is null)
        {
            return NotFound();
        }

        var command = _mapper.Map<UpdatePayment>(request) with { Id = id };
